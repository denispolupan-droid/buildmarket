/**
 * Етап 8 фасетів — родина «Захист дерева» (WOOD_CATS). Разовий переклад карток
 * після перезаливки словника (seed-char-dictionary):
 *  1. «Тип» := product_type картки (Лазур / Антисептична / Вогнебіозахист / Захисне просочення).
 *  2. «Призначення» — канонізація до закритого списку родини; без збігу — дефолт категорії.
 *  3. «Тип використання» — дефолт (у родини не було взагалі); «Основа» — канонізація.
 *
 *   npx tsx --env-file=.env.test  scripts/supabase/facet-wood.mts            (dry-run)
 *   npx tsx --env-file=.env.local scripts/supabase/facet-wood.mts --apply
 * Бекап — scripts/supabase/backups/facet-wood-*.json.
 */
...
import { WOOD_CATS } from './char-dictionary.mjs';
...
async function main() {
  ...
  if (!dict.values.get('Призначення')!.some(v => v.value === 'Декоративне тонування деревини')) throw new Error('Немає правил родини — спершу seed-char-dictionary.mjs');

  const products = await fetchAll<Product>('products', 'sku, name, category_slug, product_type', q => q.in('category_slug', WOOD_CATS));
  ...
  const defRows = await fetchAll<{ category_slug: string; default_value: string | null; characteristic_definitions: { label: string } | null }>(
    'category_characteristics', 'category_slug, default_value, characteristic_definitions(label)', q => q.in('category_slug', WOOD_CATS), 'category_slug');
  ...
  for (const p of products) {
    ...
    for (const label of ['Тип використання', 'Основа', 'Розчинник', 'Колір', 'Ступінь блиску']) {
  ...
  const file = path.join(dir, `facet-wood-${new Date().toISOString().replace(/[:.]/g, '-')}.json`);
