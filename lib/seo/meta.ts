...
// Одиниці фасовки. Метри були пропущені, і стрічки («…, 120 мм, 10 м» проти
// «…, 120 мм, 50 м») не склеювались у варіанти: базова назва в них лишалась
// різною, тож на картці не було перемикача фасовок.
//
// «мм» перед «м» у чергуванні — інакше з «120 мм» відкусилось би тільки «м», а
// це ширина стрічки, не фасовка. «шт» і «рулон» додані наперед: у каталозі
// таких фасовок ще немає, але поява нової одиниці мовчки повторює той самий
...
/**
 * Фасовка в базових одиницях — для сортування варіантів.
 * Об'єм/вага рахуються в мл/г, довжина — в мм. Змішувати шкали безпечно:
 * порівнюються лише варіанти одного продукту, а там одиниця завжди одна.
 */
export function volumeValue(volume: string | null): number {
  if (!volume) return 0;
  const m = collapse(volume).match(new RegExp(`(\\d[\\d.,]*)\\s*(${UNIT_RE})`, 'i'));
  if (!m) return 0;
  const n = parseFloat(m[1].replace(',', '.'));
  const unit = m[2].toLowerCase();
  // Штуки й рулони — рахунок, а не міра: множник тут не потрібен, значення
  // саме по собі впорядковує фасовки (10 шт < 100 шт).
  return unit === 'л' || unit === 'кг' || unit === 'м' ? n * 1000 : n;
}
