...
/**
 * Папка бренду в сховищі картинок: "Дивоцвіт" → "dyvotsvit".
 *
 * Транслітерація тут обов'язкова. Стара разова міграція картинок різала бренд
 * регуляркою [^a-z0-9] без транслітерації — і кожен кирилічний бренд перетворився
 * на рядок із самих дефісів («Титан» → "-----"), причому бренди однакової довжини
 * склалися в одну папку. Файли не побилися лише тому, що в імені є SKU і хеш.
 */
export function brandFolder(brand: string): string {
  return slugify(brand.trim()) || 'other';
}

/**
 * Слаг бренду для URL (/shop/brand/<slug>, /shop/<категорія>/<slug>).
 *
 * Раніше це було lowercase + дефіси без транслітерації в десяти копіях по
 * коду: латинські бренди працювали, а кирилічні («Сталь», «Титан», «Дивоцвіт»)
 * давали кирилічні адреси, які сторінка бренду не впізнавала — 404 у
 * percent-encoded вигляді і 500 у сирому, при цьому 12 таких URL лежали в
 * sitemap. Для латинських назв результат збігається зі старим слагом
 * (перевірено на всіх 34 брендах), тож робочі адреси не змінюються.
 */
export function brandSlug(brand: string): string {
  return slugify(brand.trim());
}

/** Старий слаг бренду (без транслітерації) — лише щоб 308-нути старі адреси на нові. */
