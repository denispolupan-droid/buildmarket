            ...
            return (
              ...
              >
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: '6px', marginBottom: '10px' }}>
                  <Link href={`${lang === 'ru' ? '/ru' : ''}/product/${p.slug ?? p.sku}`} className="pc-name" style={{ textDecoration: 'none', color: 'inherit' }} title={relDisplayName}>{relDisplayName}</Link>
                ...
                <Link href={`${lang === 'ru' ? '/ru' : ''}/product/${p.slug ?? p.sku}`} className="pc-top" style={{ textDecoration: 'none', display: 'flex' }}>
