...
// ── Fonts (bundled locally — no CDN requests at runtime) ─────────────────────
function loadFont(bold = false): Buffer {
  const name = bold ? 'DejaVuSans-Bold.ttf' : 'DejaVuSans.ttf';
  return readFileSync(join(process.cwd(), 'public', 'fonts', name));
}

// ── Bootstrap icon paths (hardcoded — no CDN requests at runtime) ─────────────
const IC_PHONE = ['M1.885.511a1.745 1.745 0 0 1 2.61.163L6.29 2.98c.329.423.445.974.315 1.494l-.547 2.19a.68.68 0 0 0 .178.643l2.457 2.457a.68.68 0 0 0 .644.178l2.189-.547a1.75 1.75 0 0 1 1.494.315l2.306 1.794c.829.645.905 1.87.163 2.611l-1.034 1.034c-.74.74-1.846 1.065-2.877.702a18.6 18.6 0 0 1-7.01-4.42 18.6 18.6 0 0 1-4.42-7.009c-.362-1.03-.037-2.137.703-2.877z'];
const IC_MAIL  = ['M.05 3.555A2 2 0 0 1 2 2h12a2 2 0 0 1 1.95 1.555L8 8.414zM0 4.697v7.104l5.803-3.558zM6.761 8.83l-6.57 4.027A2 2 0 0 0 2 14h12a2 2 0 0 0 1.808-1.144l-6.57-4.027L8 9.586zm3.436-.586L16 11.801V4.697z'];
const IC_GLOBE = ['M0 8a8 8 0 1 1 16 0A8 8 0 0 1 0 8m7.5-6.923c-.67.204-1.335.82-1.887 1.855q-.215.403-.395.872c.705.157 1.472.257 2.282.287zM4.249 3.539q.214-.577.481-1.078a7 7 0 0 1 .597-.933A7 7 0 0 0 3.051 3.05q.544.277 1.198.49zM3.509 7.5c.036-1.07.188-2.087.436-3.008a9 9 0 0 1-1.565-.667A6.96 6.96 0 0 0 1.018 7.5zm1.4-2.741a12.3 12.3 0 0 0-.4 2.741H7.5V5.091c-.91-.03-1.783-.145-2.591-.332M8.5 5.09V7.5h2.99a12.3 12.3 0 0 0-.399-2.741c-.808.187-1.681.301-2.591.332zM4.51 8.5c.035.987.176 1.914.399 2.741A13.6 13.6 0 0 1 7.5 10.91V8.5zm3.99 0v2.409c.91.03 1.783.145 2.591.332.223-.827.364-1.754.4-2.741zm-3.282 3.696q.18.469.395.872c.552 1.035 1.218 1.65 1.887 1.855V11.91c-.81.03-1.577.13-2.282.287zm.11 2.276a7 7 0 0 1-.598-.933 9 9 0 0 1-.481-1.079 8.4 8.4 0 0 0-1.198.49 7 7 0 0 0 2.276 1.522zm-1.383-2.964A13.4 13.4 0 0 1 3.508 8.5h-2.49a6.96 6.96 0 0 0 1.362 3.675c.47-.258.995-.482 1.565-.667m6.728 2.964a7 7 0 0 0 2.275-1.521 8.4 8.4 0 0 0-1.197-.49 9 9 0 0 1-.481 1.078 7 7 0 0 1-.597.933M8.5 11.909v3.014c.67-.204 1.335-.82 1.887-1.855q.216-.403.395-.872A12.6 12.6 0 0 0 8.5 11.91zm3.555-.401c.57.185 1.095.409 1.565.667A6.96 6.96 0 0 0 14.982 8.5h-2.49a13.4 13.4 0 0 1-.437 3.008M14.982 7.5a6.96 6.96 0 0 0-1.362-3.675c-.47.258-.995.482-1.565.667.248.92.4 1.938.437 3.008zM11.27 2.461q.266.502.482 1.078a8.4 8.4 0 0 0 1.196-.49 7 7 0 0 0-2.275-1.52c.218.283.418.597.597.932m-.488 1.343a8 8 0 0 0-.395-.872C9.835 1.897 9.17 1.282 8.5 1.077V4.09c.81-.03 1.577-.13 2.282-.287z'];
...
export async function buildPdf(
  ...
  opts: { showBrand: boolean; showDescriptions: boolean; showImages: boolean; priceLabel: string; headerVariant?: 'fixline' | 'fop'; isCostPrice?: boolean },
  imageBuffers: Map<string, Buffer>,
): Promise<Buffer> {
  const fontReg  = loadFont(false);
  const fontBold = loadFont(true);
  const [icPhone, icMail, icGlobe] = [IC_PHONE, IC_MAIL, IC_GLOBE];

  const { showBrand, showDescriptions, showImages, headerVariant = 'fixline', isCostPrice = false } = opts;
  // Cost prices come straight from purchase invoices at all sorts of precisions
  // (641.4, 1169.13, 705) — force a consistent 2-decimal look for that price type only.
  const fmtPrice = (v: number) => isCostPrice
  ...
  return new Promise((resolve, reject) => {
    ...
    function drawPageHeader() {
      ...
      if (headerVariant === 'fop') {
        doc.font('B').fontSize(13).fillColor('#fff')
           .text(FOP_NAME, M + 20, y0 + H / 2 - 7, { width: 220, lineBreak: false });

        doc.font('B').fontSize(13).fillColor('#fff').fillOpacity(0.95)
           .text('ПРАЙС-ЛИСТ', M, y0 + 22, { width: PW, align: 'center', lineBreak: false });
        doc.font('R').fontSize(9).fillColor('#fff').fillOpacity(0.55)
           .text(date, M, y0 + 44, { width: PW, align: 'center', lineBreak: false });
        doc.fillOpacity(1);

        const phoneText = FOP_PHONE;
        doc.font('B').fontSize(11);
        const phoneW = doc.widthOfString(phoneText);
        const iconSz = 11;
        const iconSc = iconSz / 16;
        const phoneX = M + PW - 20 - phoneW - iconSz - 6;
        const phoneY = y0 + H / 2 - 5.5;
        doc.save();
        doc.transform(iconSc, 0, 0, iconSc, phoneX, phoneY);
        for (const d of icPhone) { doc.path(d).fillColor('#fff').fill(); }
        doc.restore();
        doc.font('B').fontSize(11).fillColor('#fff')
           .text(phoneText, phoneX + iconSz + 6, phoneY - 1, { lineBreak: false });
    ...
    for (let i = 0; i < range.count; i++) {
      ...
      const footerLabel = headerVariant === 'fop' ? FOP_NAME : 'fixline.com.ua';
      doc.font('R').fontSize(7).fillColor('#9CA3AF')
         .text(`${footerLabel}  ·  Прайс-лист ${date}  ·  Стор. ${i + 1} / ${range.count}`,
