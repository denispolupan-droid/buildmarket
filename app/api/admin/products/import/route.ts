...
const KNOWN_COLUMNS = new Set([
  ...
  'Ключові слова (укр)', 'Ключові слова (рос)',
  'Фото (URL)', 'Фото (шлях)',
...
export async function GET(req: NextRequest) {
  ...
  const exampleRow: (string | number)[] = [
    ...
    85, 125, '', 150, '', 110,
    50, 'in_stock',
    'Силиконовый герметик Lacrysil 280 мл',
    'Опис укр', 'Опис рус', 'Повний опис укр', 'Повний опис рус',
    'герметик силіконовий, герметик для ванни, санітарний герметик',
    'силиконовый герметик, герметик для ванной, санитарный герметик',
...
export async function POST(req: NextRequest) {
  ...
  for (let i = 0; i < rows.length; i++) {
    ...
    const product: Record<string, unknown> = {
      ...
      name_ru:             col(row, 'Назва (рос.)') || null,
      ...
      keywords:            col(row, 'Ключові слова (укр)') || null,
      keywords_ru:         col(row, 'Ключові слова (рос)') || null,
