    ...
    <>
      ...
      <div style={{ background: 'var(--bg-soft)', minHeight: '100vh' }}>
        ...
        <div className="page-container" style={{ padding: '48px 16px 64px' }}>
          ...
          {lead && (
            <Reveal>
              <Link href={`/ru/blog/${lead.slug}`} className="blog-card blog-lead" style={{
                ...
                {lead.image && (
                  <div style={{ position: 'relative', minHeight: '260px' }}>
                    <Image src={lead.image} alt={lead.titleRu} fill sizes="(max-width: 900px) 100vw, 55vw" priority style={{ objectFit: 'cover' }} />
                  </div>
                )}
                <div style={{ padding: '32px 34px', display: 'flex', flexDirection: 'column', justifyContent: 'center' }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '14px', flexWrap: 'wrap' }}>
                    ...
                    <span style={{ fontSize: '12px', color: 'var(--text-muted)', display: 'flex', alignItems: 'center', gap: '4px' }}>
                      <Clock size={12} /> {lead.readTime} мин чтения
                    </span>
                  </div>
          ...
          <div className="blog-grid" style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '20px' }}>
            {rest.map((article, i) => (
              <Reveal key={article.slug} delay={(i % 3) * 70}>
                <Link href={`/ru/blog/${article.slug}`} className="blog-card" style={{
                  ...
                  {article.image && (
                    <div style={{ position: 'relative', aspectRatio: '1200 / 630' }}>
                      <Image src={article.image} alt={article.titleRu} fill sizes="(max-width: 700px) 100vw, (max-width: 1100px) 50vw, 33vw" style={{ objectFit: 'cover' }} />
                    </div>
                  )}
                  <div style={{ padding: '20px 22px 22px', display: 'flex', flexDirection: 'column', flex: 1 }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '10px', flexWrap: 'wrap' }}>
                      ...
                      <span style={{ fontSize: '11px', color: 'var(--text-muted)', display: 'flex', alignItems: 'center', gap: '4px' }}>
                        <Clock size={11} /> {article.readTime} мин
                      </span>
                    </div>
