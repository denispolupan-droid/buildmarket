...
const CAT_BULLETS: Record<string, string[]> = {
  ...
  'klei':               ['Клеї для плитки та монтажний клей','Клей для шпалер з індикатором','ПВА та столярний клей (D2-D4)','Суперклей, епоксидний, контактний'],
    ...
    <div style={{
      ...
      <div style={{ background: 'var(--bg-card)', display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
        ...
        {product ? (<>
            ...
            <div style={{ flexShrink: 0, textAlign: 'right', display: 'flex', flexDirection: 'column', justifyContent: 'space-between' }}>
              <div>
                ...
                <div style={{ display: 'flex', alignItems: 'baseline', gap: '5px', justifyContent: 'flex-end' }}>
                  ...
                  {priceUnit > 0 ? <span style={{ fontSize: '24px', fontWeight: 800, color: 'var(--text-primary)', lineHeight: 1 }}>{priceUnit} грн</span> : <span style={{ fontSize: '13px', color: '#94A3B8' }}>За запитом</span>}
                </div>
              </div>
          ...
          {/* Фото товару */}
          <Link href={prodHref(product.sku)} style={{ flex: 1, minHeight: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '8px 24px', background: '#fff', overflow: 'hidden' }}>
          ...
          <div style={{ padding: '12px 24px 24px', flexShrink: 0, display: 'flex', gap: '8px', alignItems: 'center', borderTop: '1px solid var(--border)' }}>
            ...
            <button onClick={handleAddToCart} disabled={!inStock} className={inStock && !cartAdded ? 'btn-primary' : undefined} style={{ flex: 1, height: '44px', borderRadius: '10px', background: cartAdded ? '#16A34A' : !inStock ? '#E2E8F0' : '#4880B8', color: !inStock ? '#94A3B8' : '#fff', border: 'none', fontSize: '14px', fontWeight: 700, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '6px', cursor: !inStock ? 'default' : 'pointer', transition: 'background 0.2s' }}>
              {cartAdded ? <><Check size={15} strokeWidth={2.5} /> Додано</> : !inStock ? 'Немає в наявності' : <><Plus size={15} strokeWidth={2.5} /> В кошик</>}
            </button>
