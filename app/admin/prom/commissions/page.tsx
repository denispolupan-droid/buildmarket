...
export default async function PromCommissionsPage() {
  ...
  const [{ data: categories }, { data: planRow }, { data: commissionRow }] = await Promise.all([
    db.from('categories')
  ...
  const commissionPct = parseFloat(commissionRow?.value ?? '3');

  return <PromCommissionsClient categories={categories ?? []} plan={plan} commissionPct={commissionPct} />;
}
