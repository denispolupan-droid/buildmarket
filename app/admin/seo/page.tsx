...
export default async function SeoQueuePage() {
  ...
  const [{ data: products }, { data: charRows }, faqRes] = await Promise.all([
    serviceClient
      .from('products')
      .select('sku, name, brand, category_slug, description_full, name_ru, description_ru, keywords, image')
      .eq('is_active', true)
      .order('category_slug')
      .order('sku'),
    ...
    serviceClient.from('product_faq').select('product_sku'),
  ]);

  const hasChars = new Set((charRows ?? []).map(r => r.product_sku));
  const hasFaq = new Set((faqRes.error ? [] : faqRes.data ?? []).map(r => r.product_sku));
