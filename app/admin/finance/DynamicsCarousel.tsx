...
//   Тижні     — стовпчики виручки/прибутку по календарних тижнях
//   Структура — кругова: куди дівається гривня виручки (факт періоду)
//   План      — накопичення виручки місяця проти рівномірного темпу плану
// Все — власний SVG, без бібліотек, кольори лише семантичні.
...
/* ── Тижні: парні стовпчики (виручка/прибуток), від'ємний прибуток — вниз ── */
  ...
  return (
    <svg viewBox={`0 0 ${W} ${H}`} style={{ width: '100%', height: 'auto' }} role="img" aria-label="Виручка і прибуток по тижнях">
      ...
      {w.labels.map((l, i) => {
        ...
        return (
          <g key={i}>
            <rect x={cx - barW - 2} y={y(Math.max(rv, 0))} width={barW} height={Math.max(2, Math.abs(y(rv) - y0))} rx="3" fill="var(--brand-blue)" opacity="0.9">
              <title>{`Тиждень з ${l}: виручка ${fmt(rv)} ₴`}</title>
            </rect>
            <rect x={cx + 2} y={pv >= 0 ? y(pv) : y0} width={barW} height={Math.max(2, Math.abs(y(pv) - y0))} rx="3" fill={pv >= 0 ? '#15803D' : '#DC2626'} opacity="0.9">
              <title>{`Тиждень з ${l}: прибуток ${fmt(pv)} ₴`}</title>
            </rect>
            <text x={cx} y={H - 6} textAnchor="middle" fontSize="10.5" fill="var(--text-muted)">{l}</text>
          </g>
        );
      })}
    </svg>
...
/* ── Структура гривні: кругова діаграма з легендою ───────────────────────── */
function StructureDonut({ s }: { s: Dynamics['structure'] }) {
  const lossProfit = s.profit < 0;
  const parts = [
    { label: 'Собівартість', value: s.cogs,     color: '#64748B' },
    { label: 'Комісії МП',   value: s.fee,      color: '#EA8A00' },
    { label: 'Доставка НП',  value: s.delivery, color: '#94A3B8' },
    { label: 'Валовий прибуток', value: Math.max(0, s.profit), color: '#15803D' },
  ].filter(p => p.value > 0);
  const total = parts.reduce((a, p) => a + p.value, 0);
  if (total <= 0) {
    return <div style={{ height: 230, display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'var(--text-muted)', fontSize: 13 }}>Немає проведених продажів за період</div>;
  }
  const R = 78, SW = 30, C = 2 * Math.PI * R;
  let acc = 0;
  const marginPct = s.revenue > 0 ? Math.round(s.profit / s.revenue * 1000) / 10 : null;
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: '28px', minHeight: 230, flexWrap: 'wrap' }}>
      <svg width="220" height="220" viewBox="0 0 220 220" role="img" aria-label="Структура виручки">
        <g transform="rotate(-90 110 110)">
          {parts.map(p => {
            const frac = p.value / total;
            const seg = (
              <circle key={p.label} cx="110" cy="110" r={R} fill="none" stroke={p.color} strokeWidth={SW}
        ...
        <text x="110" y="104" textAnchor="middle" fontSize="22" fontWeight="800" fill={s.profit >= 0 ? 'var(--text-primary)' : '#DC2626'} style={{ fontVariantNumeric: 'tabular-nums' }}>
          {marginPct === null ? '—' : `${marginPct}%`}
        </text>
        <text x="110" y="122" textAnchor="middle" fontSize="11" fill="var(--text-muted)">маржа</text>
      </svg>
      <div style={{ display: 'flex', flexDirection: 'column', gap: '9px', minWidth: 0 }}>
        <div style={{ fontSize: '12.5px', color: 'var(--text-secondary)' }}>
          Виручка · факт: <b style={{ color: 'var(--text-primary)', fontVariantNumeric: 'tabular-nums' }}>{fmt(s.revenue)} ₴</b>
        </div>
        {parts.map(p => (
          <div key={p.label} style={{ display: 'flex', alignItems: 'baseline', gap: '7px', fontSize: '12.5px' }}>
            <span style={{ width: 9, height: 9, borderRadius: '50%', background: p.color, flexShrink: 0, alignSelf: 'center' }} />
            <span style={{ color: 'var(--text-secondary)' }}>{p.label}</span>
            <span style={{ fontWeight: 700, color: 'var(--text-primary)', fontVariantNumeric: 'tabular-nums' }}>
              {fmt(p.value)} ₴ <span style={{ color: 'var(--text-muted)', fontWeight: 500 }}>· {Math.round(p.value / total * 100)}%</span>
            </span>
          </div>
        ))}
        {lossProfit && (
          <div style={{ fontSize: '12px', color: '#DC2626', fontWeight: 600 }}>Період збитковий: −{fmt(Math.abs(s.profit))} ₴</div>
        )}
      </div>
    </div>
  );
...
/* ── Накопичення виручки місяця проти рівномірного темпу плану ───────────── */
function PlanCumChart({ p }: { p: Dynamics['planCum'] }) {
  ...
  return (
    <div>
      <svg viewBox={`0 0 ${W} ${H}`} style={{ width: '100%', height: 'auto' }} role="img" aria-label={`Накопичення виручки за ${p.monthLabel}`}>
        {[0, 1, 2, 3].map(i => {
          const v = (maxV * i) / 3;
          return (
            <g key={i}>
      ...
      <div style={{ fontSize: '12px', color: 'var(--text-muted)', marginTop: '4px' }}>
        {p.plan
          ? <>Факт {fmt(lastVal)} ₴ проти темпу плану {fmt(p.plan[lastIdx] ?? 0)} ₴ — {onTrack
  ...
  return (
    <div>
      <div style={{ display: 'flex', gap: '3px', marginBottom: '10px', alignItems: 'center' }}>
        ...
        {slide === 'plan' && (
          <span style={{ display: 'flex', gap: '14px', fontSize: '11.5px', color: 'var(--text-secondary)', marginLeft: 'auto' }}>
            <span><span className="fin-dot" style={{ background: 'var(--brand-blue)' }} /> Факт</span>
            <span><span className="fin-dot" style={{ background: 'var(--text-muted)' }} /> Темп плану</span>
          </span>
        )}
      </div>
      {slide === 'weeks' && <WeekBars w={data.weeks} />}
      {slide === 'structure' && <StructureDonut s={data.structure} />}
