...
type PaymentMode = 'cash' | 'transfer';
...
export default function ReceiptPaymentPanel({
  ...
  const [editing,       setEditing]       = useState(initPaid <= 0);
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
      <div>
        ...
        <div style={{ display: 'flex', borderRadius: '8px', overflow: 'hidden', border: '1px solid var(--border)' }}>
          {([{ key: 'cash', label: '💵 Готівка' }, { key: 'transfer', label: '🏦 Рахунок' }] as { key: PaymentMode; label: string }[]).map((m, i) => (
            <button key={m.key} onClick={() => setPayMode(m.key)}
              style={{ flex: 1, height: '34px', fontSize: '12px', fontWeight: 600, cursor: 'pointer', border: 'none', borderLeft: i > 0 ? '1px solid var(--border)' : 'none', background: payMode === m.key ? '#1E3A5F' : 'var(--bg-soft)', color: payMode === m.key ? '#fff' : 'var(--text-secondary)', transition: 'all 0.15s' }}>
              {m.label}
            </button>
          ))}
        </div>
      ...
      <div style={{ display: 'flex', gap: '8px' }}>
        <div style={{ flex: 1 }}><label style={lbl}>Сума, ₴</label><input style={inp} type="number" step="0.01" value={payAmount} onChange={e => setPayAmount(e.target.value)} placeholder="0.00" /></div>
        <div style={{ flex: 1 }}><label style={lbl}>Дата оплати</label><input style={inp} type="date" value={payDate} onChange={e => setPayDate(e.target.value)} /></div>
      </div>
      <button
        onClick={handlePay}
        disabled={paying || !payAmount || parseFloat(payAmount) <= 0}
        style={{ height: '40px', borderRadius: '8px', border: 'none', background: !payAmount || parseFloat(payAmount) <= 0 ? '#E2E8F0' : '#15803D', color: !payAmount || parseFloat(payAmount) <= 0 ? '#94A3B8' : '#fff', fontSize: '14px', fontWeight: 700, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '6px' }}>
  ...
  return (
    <>
      <div style={{ background: 'var(--bg-card)', border: `1px solid ${isPaid ? '#86EFAC' : 'var(--border)'}`, borderRadius: '12px', padding: '16px' }}>
        <div style={{ fontSize: '13px', fontWeight: 700, color: isPaid ? '#15803D' : 'var(--text-muted)', marginBottom: '12px', display: 'flex', alignItems: 'center', gap: '7px' }}>
          <Banknote size={15} color={isPaid ? '#15803D' : undefined} /> Оплата постачальнику
        </div>
        ...
        /* ── Форма оплати ── */
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
            {/* Показуємо вже зроблені оплати якщо є */}
            {history.length > 0 && (
        ...
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.45)', zIndex: 3000, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          <div style={{ background: 'var(--bg-card)', borderRadius: '16px', padding: '28px', width: '420px', boxShadow: '0 20px 60px rgba(0,0,0,0.25)' }}>
            ...
            <div style={{ marginBottom: '16px' }}>
              ...
              <input value={reverseReason} onChange={e => setReverseReason(e.target.value)} placeholder="Наприклад: помилкова оплата"
                style={{ ...inp }} />
            </div>
