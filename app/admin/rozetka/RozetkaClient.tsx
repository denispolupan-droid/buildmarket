...
export default function RozetkaClient({ feedUrl, hasApiKey: initialHasApiKey, maskedToken: initialMaskedToken, totalProducts, enabledProducts, catsWithId, catsWithCommission, totalCats }: Props) {
  ...
  const statCards = [
    { label: 'API токен',       value: hasApiKey ? 'Налаштовано' : 'Відсутній', ok: hasApiKey,              icon: Key,     clickable: true },
    ...
    <div style={{ padding: '28px 32px', maxWidth: 960, margin: '0 auto' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 12, marginBottom: 24 }}>
        {navLinks.map(({ href, label, desc, color, disabled }) => (
          disabled
            ? (
              <div key={label} style={{ background: '#F8FAFC', borderRadius: 10, padding: '16px 18px', border: '1px solid #E2E8F0', opacity: 0.5 }}>
